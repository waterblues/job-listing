class HangzhouController < ApplicationController
  ...
  def index
    @jobs = case params[:order]
      when 'by_lower_bound'
        Job.where(:address => "杭州").published.order('wage_lower_bound DESC').paginate(:page => params[:page], :per_page => 15)
      when 'by_upper_bound'
        Job.where(:address => "杭州").published.order('wage_upper_bound DESC').paginate(:page => params[:page], :per_page => 15)
      else
        Job.where(:address => "杭州").published.recent.paginate(:page => params[:page], :per_page => 15)
      end
  end
  ...
  def create
    ...
    if @job.save
        redirect_to hangzhou_index_path
    else
        render :new
    end
   end
