class ChanpinController < ApplicationController
  ...
  def index
    @jobs = case params[:order]
      when 'by_lower_bound'
        Job.where(:many_kind=> "产品").published.order('wage_lower_bound DESC').paginate(:page => params[:page], :per_page => 15)
      when 'by_upper_bound'
        Job.where(:many_kind=> "产品").published.order('wage_upper_bound DESC').paginate(:page => params[:page], :per_page => 15)
      else
        Job.where(:many_kind=> "产品").published.recent.paginate(:page => params[:page], :per_page => 15)
      end
  end
